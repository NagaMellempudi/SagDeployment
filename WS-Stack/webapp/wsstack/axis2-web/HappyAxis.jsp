...
<head>
    ...
    <title>Web Services Stack Validation Page</title>
...
<%!
    /*
    * Happiness tests for Web Services Stack. These look at the classpath and warn 
	* if things are missing. Normally addng this much code in a JSP page is mad
    ...
    */
    ...
    /**
     * List deployed jar files and their versions.
     * @author bgra
     */
	public void listManifest(JspWriter out) {
		
		try {
			
			Class axisClass = org.apache.axis2.AxisFault.class;
			String axisClassLocation = getLocation(out, axisClass);

			File file = null;
	        file = new File(axisClassLocation);
	        if (!file.exists()) {
	        	System.out.println("cannot find library");
	        	return;
	        }
	        File lib = file.getParentFile();
	        if(!lib.isDirectory()) {
	        	System.out.println("cannot find library");
	        	return;
	        }
	            ...
	            out.write("<thead><tr>");
	            out.write("<th style='border: .5px #A3BBFF solid;'>Library</th>");
	            out.write("<th style='border: .5px #A3BBFF solid;'>Details</th>");
	            out.write("<th style='border: .5px #A3BBFF solid;'>Version</th>");
	            out.write("</tr></thead>");
	            ...
	            for (int i = 0; i < list.length; i++) {
	        		...
	        		// parse jar name
					// TODO: find better way to separate jar name from version
					String jarName = list[i].getName();
					String jarTitle = "";
					String jarVersion = "";

					if (jarName.lastIndexOf("-") > 0) { 
						
						jarTitle = jarName.substring(0, jarName.lastIndexOf("-"));
						jarVersion = jarName.substring(1+jarName.lastIndexOf("-"), jarName.lastIndexOf(".jar"));
						if (!(jarVersion.matches("\\d.*"))) {
							int ind = jarTitle.lastIndexOf("-");

							if (ind > 0) {
								jarTitle = jarTitle.substring(0, ind);
								jarVersion = jarName.substring(1+ind, jarName.lastIndexOf(".jar"));
							}
						}
					}
					
					// read the jar manifest for additional details
					JarFile jf = new JarFile(path);
					Manifest m = jf.getManifest();

					// in the manifest could be specified a more precise version
					// then the one in the jar file name
					
					String vendor = null;
					String title;
					String version;
					if (m != null) {
						Attributes mainAttr = m.getMainAttributes();
						vendor = mainAttr.getValue("Implementation-Vendor");
						if (vendor == null)
							vendor = mainAttr.getValue("Specification-Vendor");
						if (vendor == null)
							vendor = mainAttr.getValue("Implementation-Vendor-Id");
						title = mainAttr.getValue("Implementation-Title");
						if (title == null)
							title = mainAttr.getValue("Specification-Title");
						if (title == null)
							title = "&nbsp;";
						version = mainAttr.getValue("Implementation-Version");
						if (version == null)
							version = mainAttr.getValue("Specification-Version");
						if (version == null)
							version = jarVersion;
					...
					// two exception cases
					// in castor jar manifest the value of Implementation-Version
					// contains place-holders => rollback to the version in the jar file name
					if (jarTitle.equals("castor-xml-schema"))
						version = jarVersion;
					// it is named mex-{version}-impl => remove 'impl'
					if (jarTitle.equals("mex"))
						version = jarVersion.substring(0, jarVersion.lastIndexOf("-"));
					
					// print the results in a table
		            out.write("<tr>");
		            out.write("<th style='border: .5px #A3BBFF solid;'>" + jarName + "</th>");
		            out.write("<td style='border: .5px #A3BBFF solid;'>" + (vendor == null ? "" : "(" + 
							vendor + ") ") + title + "</td>");
		            out.write("<td style='border: .5px #A3BBFF solid;'>" + version + "&nbsp;</td>");
		            out.write("<tr>");
			...
			System.out.println("cannot list library (" + e.getMessage() + ")");
		} catch (Exception e) {
			// TODO: log reasonable message
			System.out.println("cannot list library: " + e.getMessage());
			e.printStackTrace();
		}
    ...
    private OMElement createEnvelope() {
        ...
        OMNamespace omNs = fac.createOMNamespace("http://wsstack.softwareag.com", "ns1");
...
<h1>Web Services Stack Validation Page</h1>
...
<%
    ...
    needed = needClass(out, "org.apache.axis2.transport.http.AxisServlet",
            "axis2-1.0.jar",
            "Apache-Axis",
            "Web Services Stack will not work",
...
<%
    ...
    */
    ...
    if (needed == 0) {
        //yes, be happy
        out.write("<p><font color='green'><strong>The core Web Services Stack libraries are present.</strong></font></p>");
    } else {
        //no, be very unhappy
        response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
        out.write("<font color='red'><i>"
                + needed
                + " core Web Services Stack librar"
...
<p>
    ...
    there is no guarantee your Web Services Stack Service will work, because there are many configuration options
...
<blockquote>
    <font color="green"><strong>
        Found Web Services Stack default Version service and Web Services Stack is working
        properly.</strong></font>
    <p>Now you can drop a service archive in wsstack/WEB-INF/services.
        Following output was produced while invoking Web Services Stack version service
        </p>
        <p><%= value%></p>
</blockquote>
...
%>
<p>
    <font color="brown"> There was a problem in Web Services Stack version service, may be
...
<h2>List Of Deployed Libraries</h2>
<%
	/**
	 * List Deployed Libraries
	 */
    listManifest(out);
%>
